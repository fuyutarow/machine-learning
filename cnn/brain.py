...
x_data = np.concatenate([ [ np.asarray(Image.open(dir_name+f).resize((256, 256), Image.LANCZOS))] for f in os.listdir(dir_name) if '.jpg' in f ])
y_data = np.array([ [labeler(f)] for f in os.listdir(dir_name) if '.jpg' in f])
